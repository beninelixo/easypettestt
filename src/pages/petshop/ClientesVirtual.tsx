...
import { Users, Search, Plus, Eye, PawPrint } from "lucide-react";
...
// Memoizar o componente de card individual
const ClientCard = memo(({ client, onClick }: { client: any; onClick: () => void }) => (
  <Card className="hover:shadow-lg transition-all mx-2 mb-4">
    <CardHeader>
...
const Clientes = () => {
  ...
  const handleNavigateToClient = useCallback((clientId: string) => {
    navigate(`/petshop-dashboard/cliente/${clientId}`);
  }, [navigate]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Clients List with Virtual Scrolling */}
      {loading ? (
        <Card>
